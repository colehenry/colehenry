...
2 jokers, fixed bottom-two opening peek, 3s snap window, caller penalty off,
shared-win ties, single-round matches.
"""
...
class CambioConfig:
    ...
    tie_rule: str = "shared"  # "shared" | "sudden_death" (redeal handled by room)
    match_mode: str = "single"  # "single" | "cumulative"
    match_threshold: int = 100
    # Powers only fire when the played card came off the stock; a card taken
    # from the discard is public and re-playing it for its power would loop.
    power_on_discard_draw: bool = False
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "power_on_discard_draw": self.power_on_discard_draw,
        }
